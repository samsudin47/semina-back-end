...
const updateTalents = async(req) => {
    ...
    const checkPembicara = await Talents.findOne({
        _id: id,
    });

    // jika id result false/null maka akan menampilkan error `Tidak ada pembicara dengan id` yang dikirim client
    if (!checkPembicara) throw new NotFoundError(`Tidak ada pembicara dengan id: ${id}`);

    // cari talents dengan field name dan id selain dari yang di kirim dari params
