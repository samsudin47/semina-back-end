...
const updateCategories = async(req) => {
    ...
    // cari categories dengan file name dan id selain dari yang dikirim dari params
    ...
    // apabila check true / data categories sudah maka kita tampilkan error bad request dengan message kategori nama duplikat
    if (check) throw new BadRequestError("kategori nama duplikat");

    const result = await Categories.findByIdAndUpdate({ _id: id }, { name }, { new: true, runValidators: true });

    // jika id result false / null maka akan menampilkan error `Tidak ada kategori dengan id client`
    if (!result) throw new NotFoundError(`Tidak ada kategotri dengan id: ${id}`);
